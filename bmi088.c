...
static bmi088_err_t bmi088_acc_bus_initialize(bmi088_t const *bmi088) {
  return bmi088->interface.acc_bus_initialize
             ? bmi088->interface.acc_bus_initialize(
                   bmi088->interface.acc_bus_user)
             : BMI088_ERR_NULL;
}

static bmi088_err_t bmi088_acc_bus_deinitialize(bmi088_t const *bmi088) {
  return bmi088->interface.acc_bus_deinitialize
             ? bmi088->interface.acc_bus_deinitialize(
                   bmi088->interface.acc_bus_user)
             : BMI088_ERR_NULL;
}

static bmi088_err_t bmi088_acc_bus_write_data(bmi088_t const *bmi088,
                                              uint8_t address,
                                              uint8_t const *data,
                                              size_t data_size) {
  return bmi088->interface.acc_bus_write_data
             ? bmi088->interface.acc_bus_write_data(
                   bmi088->interface.acc_bus_user, address, data, data_size)
             : BMI088_ERR_NULL;
}

static bmi088_err_t bmi088_acc_bus_read_data(bmi088_t const *bmi088,
                                             uint8_t address, uint8_t *data,
                                             size_t data_size) {
  return bmi088->interface.acc_bus_read_data
             ? bmi088->interface.acc_bus_read_data(
                   bmi088->interface.acc_bus_user, address, data, data_size)
             : BMI088_ERR_NULL;
}

static bmi088_err_t bmi088_gyro_bus_initialize(bmi088_t const *bmi088) {
  return bmi088->interface.gyro_bus_initialize
             ? bmi088->interface.gyro_bus_initialize(
                   bmi088->interface.gyro_bus_user)
             : BMI088_ERR_NULL;
}

static bmi088_err_t bmi088_gyro_bus_deinitialize(bmi088_t const *bmi088) {
  return bmi088->interface.gyro_bus_deinitialize
             ? bmi088->interface.gyro_bus_deinitialize(
                   bmi088->interface.gyro_bus_user)
             : BMI088_ERR_NULL;
}

static bmi088_err_t bmi088_gyro_bus_write_data(bmi088_t const *bmi088,
                                               uint8_t address,
                                               uint8_t const *data,
                                               size_t data_size) {
  return bmi088->interface.gyro_bus_write_data
             ? bmi088->interface.gyro_bus_write_data(
                   bmi088->interface.gyro_bus_user, address, data, data_size)
             : BMI088_ERR_NULL;
}

static bmi088_err_t bmi088_gyro_bus_read_data(bmi088_t const *bmi088,
                                              uint8_t address, uint8_t *data,
                                              size_t data_size) {
  return bmi088->interface.gyro_bus_read_data
             ? bmi088->interface.gyro_bus_read_data(
                   bmi088->interface.gyro_bus_user, address, data, data_size)
             : BMI088_ERR_NULL;
}

bmi088_err_t bmi088_initialize(bmi088_t *bmi088, bmi088_config_t const *config,
  ...
  bmi088_err_t err = bmi088_acc_bus_initialize(bmi088);
  err |= bmi088_gyro_bus_initialize(bmi088);
...
bmi088_err_t bmi088_deinitialize(bmi088_t *bmi088) {
  ...
  bmi088_err_t err = bmi088_acc_bus_deinitialize(bmi088);
  err |= bmi088_gyro_bus_deinitialize(bmi088);
...
bmi088_err_t bmi088_get_acc_data_raw(bmi088_t const *bmi088,
                                     vec3_int16_t *raw) {
  assert(bmi088 && raw);
...
bmi088_err_t
bmi088_set_acc_softreset_reg(bmi088_t const *bmi088,
  ...
  return bmi088_acc_bus_write_data(bmi088, BMI088_ACC_REG_ADDR_ACC_SOFTRESET,
                                   &data, sizeof(data));
}
...
bmi088_err_t bmi088_set_acc_pwr_ctrl_reg(bmi088_t const *bmi088,
  ...
  return bmi088_acc_bus_write_data(bmi088, BMI088_ACC_REG_ADDR_ACC_PWR_CTRL,
                                   &data, sizeof(data));
}
...
bmi088_err_t bmi088_get_acc_pwr_ctrl_reg(bmi088_t const *bmi088,
  ...
  bmi088_err_t err = bmi088_acc_bus_read_data(
      bmi088, BMI088_ACC_REG_ADDR_ACC_PWR_CTRL, &data, sizeof(data));
...
bmi088_err_t bmi088_set_acc_pwr_conf_reg(bmi088_t const *bmi088,
  ...
  return bmi088_acc_bus_write_data(bmi088, BMI088_ACC_REG_ADDR_ACC_PWR_CONF,
                                   &data, sizeof(data));
}
...
bmi088_err_t bmi088_get_acc_pwr_conf_reg(bmi088_t const *bmi088,
  ...
  bmi088_err_t err = bmi088_acc_bus_read_data(
      bmi088, BMI088_ACC_REG_ADDR_ACC_PWR_CONF, &data, sizeof(data));
...
bmi088_err_t
bmi088_set_acc_self_test_reg(bmi088_t const *bmi088,
  ...
  return bmi088_acc_bus_write_data(bmi088, BMI088_ACC_REG_ADDR_ACC_SELF_TEST,
                                   &data, sizeof(data));
}
...
bmi088_err_t bmi088_get_acc_self_test_reg(bmi088_t const *bmi088,
  ...
  bmi088_err_t err = bmi088_acc_bus_read_data(
      bmi088, BMI088_ACC_REG_ADDR_ACC_SELF_TEST, &data, sizeof(data));
...
bmi088_err_t
bmi088_set_acc_int_map_data_reg(bmi088_t const *bmi088,
  ...
  bmi088_err_t err = bmi088_acc_bus_read_data(
      bmi088, BMI088_ACC_REG_ADDR_INT_MAP_DATA, &data, sizeof(data));

  data &= ~((0x01U << 6U) | (0x01U << 5U) | (0x01U << 4U) | (0x01U << 2U) |
            (0x01U << 1U) | 0x01U);
  ...
  err |= bmi088_acc_bus_write_data(bmi088, BMI088_ACC_REG_ADDR_INT_MAP_DATA,
                                   &data, sizeof(data));
}

bmi088_err_t
bmi088_get_acc_int_map_data_reg(bmi088_t const *bmi088,
...
bmi088_err_t
bmi088_set_acc_int2_io_ctrl_reg(bmi088_t const *bmi088,
  ...
  bmi088_err_t err = bmi088_acc_bus_read_data(
      bmi088, BMI088_ACC_REG_ADDR_INT2_IO_CTRL, &data, sizeof(data));
  ...
  err |= bmi088_acc_bus_write_data(bmi088, BMI088_ACC_REG_ADDR_INT2_IO_CTRL,
                                   &data, sizeof(data));
...
bmi088_err_t
bmi088_get_acc_int2_io_ctrl_reg(bmi088_t const *bmi088,
...
bmi088_err_t
bmi088_set_acc_int1_io_ctrl_reg(bmi088_t const *bmi088,
  ...
  bmi088_err_t err = bmi088_acc_bus_read_data(
      bmi088, BMI088_ACC_REG_ADDR_INT1_IO_CTRL, &data, sizeof(data));
  ...
  err |= bmi088_acc_bus_write_data(bmi088, BMI088_ACC_REG_ADDR_INT1_IO_CTRL,
                                   &data, sizeof(data));
...
bmi088_err_t
bmi088_get_acc_int1_io_ctrl_reg(bmi088_t const *bmi088,
...
bmi088_err_t
bmi088_set_acc_fifo_config_reg(bmi088_t const *bmi088,
  ...
  bmi088_err_t err = bmi088_acc_bus_read_data(
      bmi088, BMI088_ACC_REG_ADDR_FIFO_CONFIG_0, data, sizeof(data));
  ...
  err |= bmi088_acc_bus_write_data(bmi088, BMI088_ACC_REG_ADDR_FIFO_CONFIG_0,
                                   data, sizeof(data));
...
bmi088_err_t bmi088_set_acc_fifo_wtm_reg(bmi088_t const *bmi088,
  ...
  bmi088_err_t err = bmi088_acc_bus_read_data(
      bmi088, BMI088_ACC_REG_ADDR_FIFO_WTM_0, data, sizeof(data));
  ...
  err |= bmi088_acc_bus_write_data(bmi088, BMI088_ACC_REG_ADDR_FIFO_WTM_0, data,
                                   sizeof(data));
...
bmi088_err_t bmi088_get_acc_fifo_wtm_reg(bmi088_t const *bmi088,
  ...
  reg->fifo_water_mark =
      (uint16_t)((data[0] & 0xFFU) | ((data[1] & 0x1FU) << 8U));

  return err;
}

bmi088_err_t
bmi088_set_acc_fifo_downs_reg(bmi088_t const *bmi088,
  ...
  bmi088_err_t err = bmi088_acc_bus_read_data(
      bmi088, BMI088_ACC_REG_ADDR_FIFO_DOWNS, &data, sizeof(data));

  data &= ~0x60U;

  data |= (reg->fifo_downs & 0x03U) << 5U;

  err |= bmi088_acc_bus_write_data(bmi088, BMI088_ACC_REG_ADDR_FIFO_WTM_0,
                                   &data, sizeof(data));
...
bmi088_err_t bmi088_set_acc_range_reg(bmi088_t const *bmi088,
  ...
  bmi088_err_t err = bmi088_acc_bus_read_data(
      bmi088, BMI088_ACC_REG_ADDR_ACC_RANGE, &data, sizeof(data));

  data &= ~0x03;

  data |= reg->acc_range & 0x03U;

  err |= bmi088_acc_bus_write_data(bmi088, BMI088_ACC_REG_ADDR_ACC_RANGE, &data,
                                   sizeof(data));
...
bmi088_err_t bmi088_set_acc_conf_reg(bmi088_t const *bmi088,
  ...
  return bmi088_acc_bus_write_data(bmi088, BMI088_ACC_REG_ADDR_ACC_CONF, &data,
                                   sizeof(data));
}
...
bmi088_err_t bmi088_get_acc_conf_reg(bmi088_t const *bmi088,
  ...
  bmi088_err_t err = bmi088_acc_bus_read_data(
      bmi088, BMI088_ACC_REG_ADDR_ACC_CONF, &data, sizeof(data));
...
bmi088_err_t bmi088_get_acc_fifo_data_reg(bmi088_t const *bmi088,
  ...
  bmi088_err_t err = bmi088_acc_bus_read_data(
      bmi088, BMI088_ACC_REG_ADDR_FIFO_DATA, &data, sizeof(data));
...
bmi088_err_t bmi088_get_acc_fifo_length_reg(bmi088_t const *bmi088,
  ...
  reg->fifo_byte_counter =
      (uint16_t)((data[0] & 0xFFU) | ((data[1] & 0x3FU) << 8U));
...
bmi088_err_t bmi088_get_acc_temp_reg(bmi088_t const *bmi088,
  ...
  bmi088_err_t err = bmi088_acc_bus_read_data(
      bmi088, BMI088_ACC_REG_ADDR_TEMP_MSB, data, sizeof(data));

  reg->temperature =
      (uint16_t)(((data[0] & 0xFFU) << 3U) | ((data[1] >> 5U) & 0x07U));
...
bmi088_err_t bmi088_get_acc_int_stat_reg(bmi088_t const *bmi088,
  ...
  bmi088_err_t err = bmi088_acc_bus_read_data(
      bmi088, BMI088_ACC_REG_ADDR_ACC_INT_STAT, &data, sizeof(data));
...
bmi088_err_t bmi088_get_acc_sensortime_reg(bmi088_t const *bmi088,
  ...
  bmi088_err_t err = bmi088_acc_bus_read_data(
      bmi088, BMI088_ACC_REG_ADDR_SENSORTIME_0, data, sizeof(data));

  reg->sensortime = (uint32_t)(data[0] & 0xFFU) | ((data[1] & 0xFFU) << 8U) |
                    ((data[2] & 0xFFU) << 16U);
...
bmi088_err_t bmi088_get_acc_z_reg(bmi088_t const *bmi088,
  ...
  bmi088_err_t err = bmi088_acc_bus_read_data(
      bmi088, BMI088_ACC_REG_ADDR_ACC_Z_LSB, data, sizeof(data));
...
bmi088_err_t bmi088_get_acc_y_reg(bmi088_t const *bmi088,
  ...
  bmi088_err_t err = bmi088_acc_bus_read_data(
      bmi088, BMI088_ACC_REG_ADDR_ACC_Y_LSB, data, sizeof(data));
...
bmi088_err_t bmi088_get_acc_x_reg(bmi088_t const *bmi088,
  ...
  bmi088_err_t err = bmi088_acc_bus_read_data(
      bmi088, BMI088_ACC_REG_ADDR_ACC_X_LSB, data, sizeof(data));
...
bmi088_err_t bmi088_get_acc_status_reg(bmi088_t const *bmi088,
  ...
  bmi088_err_t err = bmi088_acc_bus_read_data(
      bmi088, BMI088_ACC_REG_ADDR_ACC_STATUS, &data, sizeof(data));
...
bmi088_err_t bmi088_get_acc_err_reg(bmi088_t const *bmi088,
  ...
  bmi088_err_t err = bmi088_acc_bus_read_data(
      bmi088, BMI088_ACC_REG_ADDR_ACC_ERR_REG, &data, sizeof(data));
...
bmi088_err_t bmi088_get_acc_chip_id_reg(bmi088_t const *bmi088,
  ...
  bmi088_err_t err = bmi088_acc_bus_read_data(
      bmi088, BMI088_ACC_REG_ADDR_ACC_CHIP_ID, &data, sizeof(data));
...
bmi088_err_t bmi088_get_gyro_fifo_data_reg(bmi088_t const *bmi088,
  ...
  bmi088_err_t err = bmi088_gyro_bus_read_data(
      bmi088, BMI088_GYRO_REG_ADDR_FIFO_DATA, &data, sizeof(data));
...
bmi088_err_t
bmi088_set_gyro_fifo_config_reg(bmi088_t const *bmi088,
  ...
  bmi088_err_t err = bmi088_gyro_bus_read_data(
      bmi088, BMI088_GYRO_REG_ADDR_FIFO_CONFIG_0, data, sizeof(data));
  ...
  err |= bmi088_gyro_bus_write_data(bmi088, BMI088_GYRO_REG_ADDR_FIFO_CONFIG_0,
                                    data, sizeof(data));
...
bmi088_err_t
bmi088_get_gyro_fifo_config_reg(bmi088_t const *bmi088,
...
bmi088_err_t
bmi088_set_gyro_self_test_reg(bmi088_t const *bmi088,
  ...
  bmi088_err_t err = bmi088_gyro_bus_read_data(
      bmi088, BMI088_GYRO_REG_ADDR_GYRO_SELF_TEST, &data, sizeof(data));
  ...
  err |= bmi088_gyro_bus_write_data(bmi088, BMI088_GYRO_REG_ADDR_GYRO_SELF_TEST,
                                    &data, sizeof(data));
...
bmi088_err_t bmi088_set_gyro_fifo_ext_int_s_reg(
    bmi088_t const *bmi088, bmi088_gyro_fifo_ext_int_s_reg_t const *reg) {
  assert(bmi088 && reg);

  uint8_t data = {};

  bmi088_err_t err = bmi088_gyro_bus_read_data(
      bmi088, BMI088_GYRO_REG_ADDR_FIFO_EXT_INT_S, &data, sizeof(data));
  ...
  err |= bmi088_gyro_bus_write_data(bmi088, BMI088_GYRO_REG_ADDR_FIFO_EXT_INT_S,
                                    &data, sizeof(data));
...
bmi088_err_t
bmi088_get_gyro_fifo_ext_int_s_reg(bmi088_t const *bmi088,
...
bmi088_err_t
bmi088_set_gyro_fifo_wtm_en_reg(bmi088_t const *bmi088,
  ...
  return bmi088_gyro_bus_write_data(bmi088, BMI088_GYRO_REG_ADDR_FIFO_WM_EN,
                                    &data, sizeof(data));
}

bmi088_err_t
bmi088_get_gyro_fifo_wtm_en_reg(bmi088_t const *bmi088,
  ...
  bmi088_err_t err = bmi088_gyro_bus_read_data(
      bmi088, BMI088_GYRO_REG_ADDR_FIFO_WM_EN, &data, sizeof(data));
...
bmi088_err_t bmi088_set_gyro_int3_int4_io_map_reg(
    bmi088_t const *bmi088, bmi088_gyro_int3_int4_io_map_reg_t const *reg) {
  assert(bmi088 && reg);

  uint8_t data = {};

  bmi088_err_t err = bmi088_gyro_bus_read_data(
      bmi088, BMI088_GYRO_REG_ADDR_INT3_INT4_IO_MAP, &data, sizeof(data));
  ...
  err |= bmi088_gyro_bus_write_data(
      bmi088, BMI088_GYRO_REG_ADDR_INT3_INT4_IO_MAP, &data, sizeof(data));
...
bmi088_err_t
bmi088_get_gyro_int3_int4_io_map_reg(bmi088_t const *bmi088,
...
bmi088_err_t bmi088_set_gyro_int3_int4_io_conf_reg(
    bmi088_t const *bmi088, bmi088_gyro_int3_int4_io_conf_reg_t const *reg) {
  assert(bmi088 && reg);

  uint8_t data = {};

  bmi088_err_t err = bmi088_gyro_bus_read_data(
      bmi088, BMI088_GYRO_REG_ADDR_INT3_INT4_IO_CONF, &data, sizeof(data));
...
bmi088_err_t bmi088_get_gyro_int3_int4_io_conf_reg(
    bmi088_t const *bmi088, bmi088_gyro_int3_int4_io_conf_reg_t *reg) {
  assert(bmi088 && reg);
...
bmi088_err_t
bmi088_set_gyro_int_ctrl_reg(bmi088_t const *bmi088,
  ...
  bmi088_err_t err = bmi088_gyro_bus_read_data(
      bmi088, BMI088_GYRO_REG_ADDR_GYRO_INT_CTRL, &data, sizeof(data));
  ...
  err |= bmi088_gyro_bus_write_data(bmi088, BMI088_GYRO_REG_ADDR_GYRO_INT_CTRL,
                                    &data, sizeof(data));
...
bmi088_err_t
bmi088_set_gyro_softreset_reg(bmi088_t const *bmi088,
  ...
  return bmi088_gyro_bus_write_data(bmi088, BMI088_GYRO_REG_ADDR_GYRO_SOFTRESET,
                                    &data, sizeof(data));
}
...
bmi088_err_t bmi088_set_gyro_lpm1_reg(bmi088_t const *bmi088,
  ...
  return bmi088_gyro_bus_write_data(bmi088, BMI088_GYRO_REG_ADDR_GYRO_LPM1,
                                    &data, sizeof(data));
}
...
bmi088_err_t bmi088_get_gyro_lpm1_reg(bmi088_t const *bmi088,
  ...
  bmi088_err_t err = bmi088_gyro_bus_read_data(
      bmi088, BMI088_GYRO_REG_ADDR_GYRO_LPM1, &data, sizeof(data));
...
bmi088_err_t
bmi088_set_gyro_bandwidth_reg(bmi088_t const *bmi088,
  ...
  return bmi088_gyro_bus_write_data(bmi088, BMI088_GYRO_REG_ADDR_GYRO_BANDWIDTH,
                                    &data, sizeof(data));
}
...
bmi088_err_t bmi088_get_gyro_bandwidth_reg(bmi088_t const *bmi088,
  ...
  bmi088_err_t err = bmi088_gyro_bus_read_data(
      bmi088, BMI088_GYRO_REG_ADDR_GYRO_BANDWIDTH, &data, sizeof(data));
...
bmi088_err_t bmi088_set_gyro_range_reg(bmi088_t const *bmi088,
  ...
  return bmi088_gyro_bus_write_data(bmi088, BMI088_GYRO_REG_ADDR_GYRO_RANGE,
                                    &data, sizeof(data));
}
...
bmi088_err_t bmi088_get_gyro_range_reg(bmi088_t const *bmi088,
  ...
  bmi088_err_t err = bmi088_gyro_bus_read_data(
      bmi088, BMI088_GYRO_REG_ADDR_GYRO_RANGE, &data, sizeof(data));
...
bmi088_err_t
bmi088_get_gyro_fifo_status_reg(bmi088_t const *bmi088,
  ...
  bmi088_err_t err = bmi088_gyro_bus_read_data(
      bmi088, BMI088_GYRO_REG_ADDR_FIFO_STATUS, &data, sizeof(data));
...
bmi088_err_t bmi088_get_get_gyro_int_stat_reg(bmi088_t const *bmi088,
  ...
  bmi088_err_t err = bmi088_gyro_bus_read_data(
      bmi088, BMI088_GYRO_REG_ADDR_GYRO_INT_STAT, &data, sizeof(data));
...
bmi088_err_t bmi088_get_gyro_rate_z_reg(bmi088_t const *bmi088,
  ...
  bmi088_err_t err = bmi088_gyro_bus_read_data(
      bmi088, BMI088_GYRO_REG_ADDR_RATE_Z_LSB, data, sizeof(data));
...
bmi088_err_t bmi088_get_gyro_rate_y_reg(bmi088_t const *bmi088,
  ...
  bmi088_err_t err = bmi088_gyro_bus_read_data(
      bmi088, BMI088_GYRO_REG_ADDR_RATE_Y_LSB, data, sizeof(data));
...
bmi088_err_t bmi088_get_gyro_rate_x_reg(bmi088_t const *bmi088,
  ...
  bmi088_err_t err = bmi088_gyro_bus_read_data(
      bmi088, BMI088_GYRO_REG_ADDR_RATE_X_LSB, data, sizeof(data));
...
bmi088_err_t bmi088_get_gyro_chip_id_reg(bmi088_t const *bmi088,
  ...
  bmi088_err_t err = bmi088_gyro_bus_read_data(
      bmi088, BMI088_GYRO_REG_ADDR_GYRO_CHIP_ID, &data, sizeof(data));
